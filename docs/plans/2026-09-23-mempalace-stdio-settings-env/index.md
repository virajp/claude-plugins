...
| 2    | U3 docs       | opus  | 1     | pass        | node edit; 11 files + new decisions doc; heading renamed to `Running the server (stdio)`, anchor re-pointed; DECIDED grep sweep in place of a docs-sync call (survey had listed targets); GAP `.claude/docs/plugins.md:12` said HTTP though the survey marked `.claude/docs/` clean — fixed inside Owns                               | —        |
