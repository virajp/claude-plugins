...
- **Model:** opus
- **Read first:** the consent block in `index.md`; the three version files.
...
> Release vwf publicly — minor — `plugins/vwf/.claude-plugin/plugin.json`, the
> next minor above the value the tree holds when U7 runs, never a 13 or 17
...
> the tree holds when U7 runs, never a 13 or 17 component; by editing the
> `version` field.

> Release site publicly — patch — `mise run p:site:version` (bare; patch is its
> default; it refuses a dirty tree, so U7 runs it first).
...
2. **`plugins/vwf/.claude-plugin/plugin.json`** — edit `version` to the next
   minor above the value the file holds now: `X.Y.Z` becomes `X.(Y+1).0`, and if
   `Y+1` is 13 or 17 it becomes `X.(Y+2).0`. Plain semver.
3. **`plugins/stackgen/.claude-plugin/plugin.json`** — the same rule.
4. **`.claude-plugin/marketplace.json`** — run `mise run p:plugins:marketplace`
...
- `command grep -n '"version"' plugins/vwf/.claude-plugin/plugin.json plugins/stackgen/.claude-plugin/plugin.json`
  shows two new minors with no 13 or 17 component.
- `command grep -rn -i 'claude-code\|taste-skill' plugins/vwf` is empty.
- `command grep -l '^default: true' plugins/stackgen/stacks/bundles/*.md` lists
  exactly `claude-code.md`.

## Guardrails

- Do not touch any file outside the four owned.
...
`ops: bump vwf and stackgen (minor) and site (patch) — terminal design tool` —
written by the orchestrator after the wave gate, not by the unit. Type `ops` is
