...
COMPLETE 2026-09-27 — b9ec8de8..7d05a166 (41 commits) on
2026-09-26-tool-config-gates; 25 gaps open, folder left live
...
| U8  | 2    | [08-assets.md](08-assets.md)                   | edit   | `plugins/stackgen/skills/tool-config/assets/{dprint,pre-commit,gitleaks,grype}/**`, `plugins/stackgen/skills/tool-config/assets/mise/.config/mise/tasks/setup/precommit`, the prose of `plugins/stackgen/stacks/bundles/mise.md`, widened by the scratch gate: `plugins/stackgen/skills/tool-config/assets/mise/.config/mise/tasks/code/graph`, `plugins/stackgen/skills/tool-config/assets/mise/.config/mise.toml`, widened in wave 4: `plugins/stackgen/skills/tool-config/assets/mise/.config/mise/tasks/setup/mise`                                                                                                                                  | U1                     | green  | 78389403 |
| U9  | 3    | [09-review.md](09-review.md)                   | review | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | U5, U8, U12            | green  |          |
| U10 | 4    | [10-docs.md](10-docs.md)                       | edit   | `.claude/**`, `CLAUDE.md`, `readme.md`, `site/src/content/docs/**`, `docs/memory/decisions/2026-09-26-tool-config-gates.md` (new), widened by R1 (rule 5): `plugins/stackgen/agents/stackgen-skill-reviewer.md`, `plugins/stackgen/stacks/repo-hygiene/repo-hygiene/conventions.md`, `plugins/stackgen/stacks/toolchain-gate/{eslint,ruff}/conventions.md`, `plugins/stackgen/stacks/package-manager/uv/conventions.md`, `plugins/stackgen/stacks/package-manager/swiftpm/skills/swiftpm/SKILL.md`, widened by U9: `plugins/stackgen/stacks/package-manager/pnpm/conventions.md`, `plugins/stackgen/stacks/toolchain-gate/eslint/skills/eslint/SKILL.md` | U2, U3, U4, U6, U7, U9 | green  | 8acddbd6 |
| U11 | 5    | [11-gates-and-bump.md](11-gates-and-bump.md)   | edit   | `plugins/stackgen/stacks/inventory.md`, `.claude-plugin/marketplace.json`, the `version:` line of every pack U3 edited, every `plugins/stackgen/stacks/bundles/*.md` pin naming them                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | U10                    | green  | 7d05a166 |
...
16. **After-landing hook swap lacks `--config`** (U9 round 3, orchestrator) —
    `pre-commit install --hook-type post-commit` writes a hook pointing at a
    root `.pre-commit-config.yaml` this repo does not have, so every commit
    would error and `graphify-refresh` never runs. The run will not execute it
    as written: it is offered as an `ask` with
    `--config .config/pre-commit-config.yaml` added.
17. **No refresh after a merge landing** (U9 round 3, plan gap) — decision 5
    hooks only `post-commit`; `git merge --no-ff` in the main checkout fires
    `post-merge`, and the raw `post-checkout` hook that covered it is removed.
18. **bun repos get no `node_modules` exclude** (U9 round 3, plan gap) —
    decision 3 gives it to the pnpm pack only; `bun@generated` has no rule
    requiring the request, so gitleaks walks `node_modules/` in a bun repo.
19. **U9 contested at the round cap** (wave 3, cap) — low: `code/graph:118`
    writes `$!` without checking the lock is still its own, and a pid reused
    after a reboot holds the lock forever (U8, U12's copy); `code/graph:31-36`
    misses `.git/sequencer/` and `REVERT_HEAD` (U8, U12); `check.ts:808` counts
    a `rev=` inside an https URL as the pin (U5); `setup/precommit:132` never
    checks `post-commit.legacy`, and a failing `graphify hook uninstall` under
    `set -e` leaves no hooks (U8).
20. **The installer still runs `graphify hook install`** (U9 engine, out of
    scope) — `installer/src/graphify.ts:69` installs the raw hook this plan
    replaces, in any git repo; the plan leaves the installer untouched.
21. **U9-late1 findings on uncovered U2, dropped** (wave 3) — `add hook` at
    stage `post-commit` does not require `always_run=true`, so such a hook never
    fires (`references/pre-commit.md:205`; the checker's `hookFault` does not
    enforce it either); `grype add ignore <id> [reason]` does not say how a
    reason ending `for <word>` is told from a requester suffix
    (`references/grype.md:45`).
...
24. **This repo's `setup:precommit` lacks the raw-hook strip** (U9-late2,
    unmapped, plan gap) — decision 9 gives this repo the refresh hook, but no
    unit owns `.config/mise/tasks/setup/precommit`, so it never re-copied the
    asset's strip; `pre-commit install --overwrite` leaves graphify's raw
    `post-checkout` hook. The after-landing `graphify hook uninstall` covers
    this machine; another clone would not be.
25. **A long pack hook line is re-wrapped by the shipped formatter** (final
    scratch gate) — uv's one-line hook `description` (118 characters) lands
    unwrapped, and the shipped dprint re-wraps it, so a freshly shaped uv repo's
    first format run touches a file nobody edited; `references/pre-commit.md`
    states no fold rule for a written hook. Other ambiguities it noted: a blank
    line before a requester block in `repos:` (SKILL.md:163-174), the lock key
    for an exclude (dprint.md:40), and whether a user's own exclude line breaks
    the "one set" promise (SKILL.md:129 vs :197).
...
| 3    | U9 review          | opus         | 3     | pass         | node security; range d49127d6..54625461; engine none high-confidence; approve                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | —                   |
| 3    | U9 review          | opus         | 3     | findings(3)  | node review; range d49127d6..54625461; covered 4 → 3 (converging); loop-back: U8 code/graph:75 stale-lock takeover not atomic (+U12 copy); U5 check.ts:723 suffix holes (grype tail passes 'for uv', 'all add exclude for pnpm' passes); orchestrator: after-landing hook swap lacks --config → gap 16, step to be asked with the corrected command; plan gaps: post-merge refresh (17), bun node_modules (18)                                                                                                                                                               | —                   |
| 3    | U5 checker         | opus         | 4     | pass         | node edit; U9 r3 fix: trailing for <pack-slug> refused on tail verbs, path 'for' refused; vitest 345/345; GAP: a grype reason ending in a pack name must be reworded                                                                                                                                                                                                                                                                                                                                                                                                         | f0fb200d            |
| 3    | U8 assets          | opus         | 8     | pass         | node edit; U9 r3 fix: stale-lock takeover via mkdir takeover mutex, re-check under it; 5×6 concurrent runs → 1 build each, nothing left behind                                                                                                                                                                                                                                                                                                                                                                                                                               | 53ba7a1e            |
| 3    | U12 this-repo      | orchestrator | 6     | pass         | orchestrator-applied edit on U12's behalf (not dispatched): code/graph cp -p from U8's asset, cmp identical                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | 27d7b259            |
| 3    | U9 review          | opus         | 4     | pass         | node security; range d49127d6..1a8758d3; engine none high-confidence; approve                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | —                   |
| 3    | U9 review          | opus         | 4     | findings(4)  | node review; range d49127d6..1a8758d3; approve; cap reached (covered 5 → 4 → 3 → 4 low) — contested: U8/U12 code/graph:118 unchecked $! write + pid reuse after reboot; U8/U12 code/graph:31-36 no sequencer/REVERT_HEAD guard; U5 check.ts:808 rev= inside URL counts; U8 setup/precommit:132 post-commit.legacy unchecked + set -e on failing uninstall. Engine-only: installer/src/graphify.ts:69 still runs graphify hook install (out of scope) → gap                                                                                                                   | —                   |
| 3    | R3                 | opus         | 1     | findings(4)  | U2: references/pre-commit.md:342 setup:precommit post-checkout + no-graphify strip and code:graph single-flight lock undocumented (also mise.md:215,:969); U2 fold SKILL.md:133, dprint.md:196; U8 linter.yaml:32 comment 113 cols; contract + rulings clean                                                                                                                                                                                                                                                                                                                 | —                   |
| 3    | U8 assets          | opus         | 9     | pass         | node edit; R3 fix: linter.yaml comments wrapped to 80 cols, entries unchanged                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | c297b5f0            |
| 3    | U2 skill           | opus         | 5     | pass         | node edit; R3 fix: pre-commit.md hook strip (post-commit + post-checkout, no-graphify path), mise.md single-flight lock; refolds SKILL.md, dprint.md                                                                                                                                                                                                                                                                                                                                                                                                                         | 4852a8ec            |
| 3    | R3                 | opus         | 2     | findings(1)  | cap reached, 4 → 1; contested: references/mise.md:224 fold breaks early (U2, cosmetic); contract + rulings clean                                                                                                                                                                                                                                                                                                                                                                                                                                                             | —                   |
| 3    | gate               | opus         | 1     | pass         | wave gate 7/7 green; late re-run U9-late1 pending over c297b5f0 (U8 fixed after U9)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | —                   |
| 3    | U9 review          | opus         | 1     | pass         | node security; re-run 1 (U9-late1), range c297b5f0^..c297b5f0; approve                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | —                   |
...
| 3    | R3-late            | opus         | 1     | pass         | late contract review over c297b5f0^..f832d767: contract + rulings clean; only the contested mise.md:224 fold (R3 round 2) carried                                                                                                                                                                                                                                                                                                                                                                                                                                            | —                   |
| 3    | gate               | opus         | 2     | pass         | wave gate re-run after U9-late1 and R3-late: 7/7 green                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | —                   |
| —    | acceptance         | —            | —     | skipped      | why: no covers: — no acceptance criteria                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | —                   |
| —    | ux                 | —            | —     | skipped      | why: no covers: — no Screens contract                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | —                   |
| —    | reconcile          | —            | —     | skipped      | why: no covers: — no stamps, registry or harness to reconcile; no code unit — nothing to persist                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | —                   |
| 4    | U10 docs           | opus         | 1     | pass         | node edit; docs-sync over d49127d6..HEAD + routed DOCS FALSIFIED: CLAUDE.md, readme.md, .claude/docs + skills, site stackgen.md/vwf.md/onboard-existing-codebase.md, decision doc, widened plugin prose (skill-reviewer agent, repo-hygiene/eslint/ruff/uv/pnpm conventions, eslint + swiftpm SKILL.md); DECIDED rule 11 = eight assertions; DOCS FALSIFIED: U8 asset setup/mise:87 'written by a gate pack'; GAP: surveyor reports misrouted, covered by own sweep                                                                                                          |                     |
| 4    | U10 docs           | opus         | 2     | pass         | node edit; surveyor relay: site stackgen.md:1212 'the gates bundle installs' → tool-config all; other surveyor items already fixed or still true                                                                                                                                                                                                                                                                                                                                                                                                                             |                     |
| 4    | R4                 | opus         | 1     | findings(3)  | U10: dependencies.md:43 'two slugs'; CLAUDE.md:168 + repo-shape.md:162 'eight assertions' vs check.ts:382 'Seven'; eslint/conventions.md:30 94 cols; contract + rulings clean. U8 setup/mise:87 (U10 DOCS FALSIFIED) looped to U8                                                                                                                                                                                                                                                                                                                                            | —                   |
| 4    | U8 assets          | opus         | 11    | pass         | node edit; wave-4 DOCS FALSIFIED fix, Owns widened (GAP) to assets/mise/.config/mise/tasks/setup/mise: :87 comment 'landed by stackgen:tool-config'; no other stale phrasing in assets                                                                                                                                                                                                                                                                                                                                                                                       |                     |
| 4    | U10 docs           | opus         | 3     | pass         | node edit; R4 fix: dependencies.md one slug; rule 11 = seven (matches check.ts:382) in CLAUDE.md, repo-shape.md, checks.md, stackgen-plugin SKILL.md; eslint conventions refolded                                                                                                                                                                                                                                                                                                                                                                                            |                     |
| 4    | R4                 | opus         | 2     | pass         | findings 3 → 0; contract + rulings clean                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | —                   |
| 4    | gate               | opus         | 1     | pass         | wave gate 7/7 green; late re-run U9-late2 pending over 9f700d9a (U8 after U9)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | —                   |
| 4    | U9 review          | opus         | 1     | pass         | node security; re-run 2 (U9-late2), range 9f700d9a^..9f700d9a; approve                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | —                   |
| 4    | U9 review          | opus         | 1     | findings(1)  | node review; re-run 2 (U9-late2), range 9f700d9a^..9f700d9a; approve; contested (unmapped): .config/mise/tasks/setup/precommit:13 this repo's copy lacks the raw-hook strip — gap 24                                                                                                                                                                                                                                                                                                                                                                                         | —                   |
| 4    | R4-late            | —            | —     | skipped      | why: U9-late2 produced no fix commits — nothing for a late contract review                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | —                   |
| 4    | gate               | opus         | 2     | pass         | wave gate re-run after U9-late2: 7/7 green                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | —                   |
| 5    | U11 gates-and-bump | opus         | 1     | pass         | node edit; 14 packs one minor up (no 13/17 skip), 43 pins across 23 bundles, inventory.md regenerated, marketplace unchanged (no plugin bump); gate 7/7 green                                                                                                                                                                                                                                                                                                                                                                                                                | 7d05a166            |
| 5    | R5                 | opus         | 1     | pass         | 14 bumps one minor, 43 pins, inventory fresh, no plugin bump; contract + rulings clean                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | —                   |
| —    | scratch gate       | opus         | 2     | pass         | orchestrator gate over the finished tree: plan conditions 1–6 pass; extra check 7 (landed files pass the shipped dprint check) fails only on uv's 118-char hook description, re-wrapped by dprint — gap 25                                                                                                                                                                                                                                                                                                                                                                   | —                   |
| —    | final gate         | opus         | 1     | pass         | wave gate 7/7 green over the finished tree                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | —                   |
| —    | landing            | opus         | 1     | pass         | Status COMPLETE; 25 gaps open → folder left live, not archived; /vwf:backlog partial B66 B72 (both Partially done, Landed lines written)                                                                                                                                                                                                                                                                                                                                                                                                                                     | —                   |
