# U7 — Gates and bump: stackgen one minor up, the generators, the full gate, target-verifier
...
- **Read first:** `plugins/stackgen/.claude-plugin/plugin.json` (the version
  **on disk now** — this plan may run before or after plans B and C);
  `plugins/stackgen/stacks/inventory.md:10` (the counts on disk now);
  `.claude/skills/release/SKILL.md` §versions (a minor bump; a tracked version
  is plain `X.Y.Z`); `.claude/docs/dev-marketplace.md` (what
...
D13: "stackgen bumps one **minor** from the version on disk at run time
(expected `1.6.0` if this plan runs last), untagged." Consent: "Release
`stackgen` publicly — none (minor bump from the version on disk recorded, no
...
1. **`plugins/stackgen/.claude-plugin/plugin.json`** — read the current
   `"version"`; write the next minor (`1.3.0` → `1.4.0`, `1.4.0` → `1.5.0`,
   `1.5.0` → `1.6.0`). Nothing else in the file. Record the before/after in
   `DECIDED:`.
...
3. **Run `mise run plugins:inventory`** — regenerates
   ...
   on-disk count before this plan **+4**, the bundle count **+4**, kinds
   unchanged at 12 (expected `62 packs, 58 bundles, 12 kinds` if A, B and C have
   all landed); the four new pack rows carry categories `media`, `realtime`,
   `messaging`, `secrets-manager`; the four bundle rows show `backing` and two
   components each. If the orchestrator already regenerated it at the wave-2
...
5. **Run `target-verifier`** (the agent, from inside this unit) with: "stackgen
   bumped to <new>; four new `cloud-service` packs (`images`, `realtime`,
   `email-service`, `secrets-store`) and four new bundles `cloudflare-images`,
   `cloudflare-realtime`, `cloudflare-email-service`,
   `cloudflare-secrets-store`; the Cloudflare provider pack's prose changed.
   Prove, hermetically (`CLAUDE_CONFIG_DIR` under `/tmp`), that installing
   stackgen from the dev marketplace lands all four pack directories and four
...
- The inventory header line reads the expected counts (+4 / +4 / 12).
...
- `plugin.json` is JSON — edit with Edit, keep the key order and trailing
  newline.
- A tracked plugin version is plain `X.Y.Z`; never write `X.Y.Z+N` here.
- On this machine `git commit` needs `mise x --` in front and heredoc bodies;
  irrelevant to you — you never commit.

## Commit

`ops: bump stackgen — the Cloudflare media, messaging and secrets packs` —
