...
- **Owns:** `plugins/stackgen/.claude-plugin/plugin.json`,
  `plugins/vwf/.claude-plugin/plugin.json`, `site/package.json`,
...
- **Lazy-load:** `.config/mise/tasks/p/site/version` (the bare invocation is a
  ...
  `.config/mise/tasks/p/plugins/inventory`.

## Ruling

The Consent block: "Release `stackgen` publicly — minor — `1.18.0` → `1.19.0`,
by editing `plugins/stackgen/.claude-plugin/plugin.json`"; "Release `vwf`
publicly — minor — `19.27.1` → `19.28.0`, by editing
`plugins/vwf/.claude-plugin/plugin.json`"; "Release `site` publicly — patch —
`1.1.20` → `1.1.21`, by `mise run p:site:version` (bare)".

Decision 4: "No `config_format` or `blueprint_format` bump".

The 13/17 rule: no component of any version this unit writes is `13` or `17`;
none of the three targets is, so no skip applies — verify rather than assume. If
the base's actual versions differ from the literals above (a plan landed in
between), apply the consented **level** over the actual version and record the
literal reached as a `GAP:`.

## Edits

1. `mise run p:site:version` — from a clean tree; confirm `site/package.json`
   reads `1.1.21`.
2. `plugins/stackgen/.claude-plugin/plugin.json` — `"version": "1.19.0"`.
3. `plugins/vwf/.claude-plugin/plugin.json` — `"version": "19.28.0"`.
...
5. `mise run p:plugins:inventory` — expected to change nothing; report if it
   does.
...
- `command grep -n '"version"' plugins/stackgen/.claude-plugin/plugin.json plugins/vwf/.claude-plugin/plugin.json site/package.json`
  shows `1.19.0`, `19.28.0`, `1.1.21`.
- `command grep -c "stackgen-v1.19.0\|vwf-v19.28.0" .claude-plugin/marketplace.json`
  is `2`.

## Guardrails

- Touch nothing outside the five owned files and what the two generators write.
...
`ops: bump stackgen 1.19.0, vwf 19.28.0, site 1.1.21 — default per platform` —
