...
COMPLETE 2026-09-20 — add917c6, 553521eb, 9ca99019, f0717dc4, 0115728f, 92d0a047
...
| U5 | 3    | [05-gates-and-bump.md](05-gates-and-bump.md)     | edit | `plugins/vwf/.claude-plugin/plugin.json`, `plugins/stackgen/.claude-plugin/plugin.json`, `site/package.json`, `.claude-plugin/marketplace.json`                                                                                                               | U4         | green  | 0115728f, 92d0a047 |
...
All four were resolved inside the run; none is open.

- **Nobody-owned falsified passages** — `setup/references/format-lineage.md`,
  `setup/references/migrate-pipeline.md` and `architecture/SKILL.md` each
  restated the one-key or format-19 claim; U2's Owns was widened to all three at
  run time (U4 may not edit `plugins/**`). Fixed in 553521eb.
- **Decision 2 versus 01-init.md edit 1** — decision 2 offers union "for the
  extensions list"; edit 1 says an identical extension id is kept without
  asking. Ruled: the unit file governs, since every choice on an identical id
  yields the same file; `editor_keys` never records an extension id, and union
  is offered for an object-valued key or a nesting parent. Applied consistently
  in 553521eb, 9ca99019 and f0717dc4.
- **Site bump ordering** — `p:site:version` refuses a dirty tree and the
  folder's own run-log edits kept it dirty; the plugin bumps landed first
  (0115728f), the site bump second (92d0a047).
- **Surveyor routing** — the docs-sync surveyor's report reached the
  orchestrator, not U4; relayed, every finding covered.
...
| 3    | U5 gates-and-bump   | opus  | 1     | pass        | node: edit; vwf 19.37.0, stackgen 1.20.3, marketplace regenerated (refs vwf-v19.37.0, stackgen-v1.20.3); four gate lines green; UNRESOLVED (mechanical): `p:site:version` refuses a dirty tree (ERR_PNPM_UNCLEAN_WORKING_TREE) and the run log keeps the tree dirty. Orchestrator ruling (GAP: plan ordered the site bump first without accounting for the folder's own edits): close the wave-2 folder commit, commit the two plugin bumps as U5's first commit, then U5 round 2 runs the task on a clean tree and the site bump lands as a second U5 commit                                                                                                                                                                                                                                                                                                                                                                                                                                                  | 0115728f |
| 3    | U5 gates-and-bump   | opus  | 2     | pass        | node: edit; site/package.json 1.1.31 → 1.1.32 via `p:site:version` on the clean tree, no lockfile change, task staged nothing; p:site:check green                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | 92d0a047 |
| 3    | R3                  | opus  | 1     | pass        | 0115728f + site/package.json only; versions as consented, no 13/17, installer untouched, marketplace refs correct, no lockfile delta, no old version named as current; CONTRACT clean; RULINGS clean                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | —        |
| —    | reconcile           | —     | 2     | pass        | final gate over the finished tree: marketplace --check, inventory --check, p:plugins:check, code:precommit (twice), p:site:check all green; Gates the orchestrator keeps: none beyond the wave gate                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | 92d0a047 |
