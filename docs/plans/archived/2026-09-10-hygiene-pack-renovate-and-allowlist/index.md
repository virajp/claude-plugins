...
| U4 | 2    | [04-docs.md](04-docs.md)                                 | `.claude/skills/stackgen-plugin/**`, `site/src/content/docs/plugins/stackgen.md`, `CLAUDE.md`, `readme.md`, `.claude/docs/**`, `docs/memory/decisions/**` (none expected), `plugins/stackgen/assets/kinds.md` (widened at run time — the `:463` passage U1 reported falsified, owned by nobody), `plugins/stackgen/skills/stackgen-stack-template/references/materializer.md` (widened at run time — R1's rule-5 finding at `:71`, owned by nobody) | U1–U3      | green  | `80acd7fd` |
| U5 | 3    | [05-gates-and-bump.md](05-gates-and-bump.md)             | `plugins/stackgen/.claude-plugin/plugin.json`, `plugins/stackgen/stacks/bundles/repo-hygiene.md`, `plugins/stackgen/stacks/bundles/repo-gates.md`, `plugins/stackgen/stacks/inventory.md`, `.claude-plugin/marketplace.json`                                                                                                                                                                                                                        | U4         | green  | `8b0d1d65` |
...
| 1    | gate      | —     | 1     | green (2 red) | Green: `plugins:check`, `plugins:marketplace --check`, `plugins:npm-normalize-test`, `tsc -p installer`, `tsc -p scripts`, `site:check`, and the plan's renovate.json move check. Red on one cause: `plugins:inventory --check` and `pnpm vitest run` (inventory.test.ts only) — `repo-gates.md` pins `dprint@1.0.0` and `repo-hygiene.md` pins `repo-hygiene@1.0.0` while both pack.yaml files read 1.0.1 (U1/U3's ruled bumps). The pins and `inventory.md` are U5's alone by the Shared-file rule, so these two lines are red by the plan's own construction until wave 3; attributed to U5 (pending), not a failure. No `UNRESOLVED:` in any report. **GAP (orchestrator):** the pre-commit `plugins-inventory` hook fires on any `stacks/` path, so U1's and U3's commits cannot pass with their `pack.yaml` bump staged while the pins are not. Assumption: each unit's commit carries its Owns minus the `pack.yaml` version line; the two version lines stay in the worktree and ride U5's gates-and-bump commit together with the pins and the regenerated inventory — the atomic set the hook enforces. Commit order was U2 → U1 → U3: the `plugins-check` hook refused U1 first (U2's unstaged checker change was stashed by pre-commit), and U1's still-staged files then rode into U2's commit; both were soft-reset and redone with exact per-unit staging. | —        |
| 2    | U4        | opus  | 1     | green         | docs-sync over the branch delta plus the five handed passages: stackgen-plugin SKILL.md (renovate path; allowlist restated as two tiers citing output-tree.md), repo-shape.md (rule 11 = landable tier), CLAUDE.md Tasks bullet (prose only, no table), manual stackgen.md (root `renovate.json`; two-tier paragraph), kinds.md :463 and materializer.md :71 (both hand-folded). DECIDED: materializer.md's "manifest or lockfile may land at the root" clause dropped — same falsification R1 found in U1. DECIDED: repo-shape.md:160 and stackgen.md:268 edited beyond the handed list — same restatement, in Owns. DECIDED: readme.md untouched (still true); the 2026-09-05 decisions doc left as a dated record. DOCS FALSIFIED (deliberately not edited): `docs/memory/decisions/2026-09-05-charter-fence-opens-for-gate-configs.md:27`. GAP: manual's reach unspecified — assumed it names both vwf-owned files explicitly. site:check, plugins:check, dprint check, grep all green.                                                                                                                                                                                                                                                                                                                                                                               | 80acd7fd |
| 2    | R2        | opus  | 1     | findings(1)   | CONTRACT clean, RULINGS clean. Rule 5: `.claude/skills/stackgen-plugin/SKILL.md:158` — rule 11's restatement still says "the root allowlist over the tier's top level" while :122-127 says only the landable tier reaches the checker → U4 round 2. Verified: six paths all in Owns; tier wording matches output-tree.md entry for entry; manual names both vwf-owned files (closes U4's GAP); dprint check clean on the formatted paths; kinds.md and materializer.md folded within their bands; the only surviving `.config/renovate.json` is the dated decisions doc.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |          |
| 2    | U4        | opus  | 2     | green         | stackgen-plugin SKILL.md:158 sharpened to the landable tier — states the vwf-owned tier never reaches the checker and a pack shipping CLAUDE.md or mempalace.yaml there is refused. DECIDED: all three rule-11 restatements (CLAUDE.md, repo-shape.md, this skill) now share the "landable tier" phrasing. dprint check and plugins:check green; no site file changed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |          |
| 2    | R2        | opus  | 2     | pass          | FINDINGS 0. :158 resolved — matches check.ts's landable set and the two wave-1 tests. Same six files, all in Owns; dprint check exit 0 on the formatted paths; frontmatter intact; no residue; both greps empty. CONTRACT clean, RULINGS clean.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |          |
| 2    | gate      | —     | 1     | green (2 red) | Green: `plugins:check`, `plugins:marketplace --check`, `plugins:npm-normalize-test`, `tsc -p installer`, `tsc -p scripts`, `site:check`, and the plan's `.config/renovate.json` grep (nothing). Red on the same one cause as wave 1: `plugins:inventory --check` and `pnpm vitest run` (inventory.test.ts only) on the U5-owned pins. No `UNRESOLVED:`. U4 committed 80acd7fd — its commit fires the inventory hook via kinds.md, but pre-commit stashes the unstaged pack.yaml bumps, so the hook sees a consistent tree.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | 80acd7fd |
| 3    | U5        | opus  | 1     | green         | plugin.json 1.6.0 → 1.6.1; repo-hygiene.md pin → @1.0.1; repo-gates.md dprint pin → @1.0.1 (other three untouched); inventory.md and marketplace.json regenerated (stackgen ref → stackgen-v1.6.1). All eight gate lines and both plan checks PASS by the unit, re-run by the orchestrator: inventory --check, vitest (293), plugins:check, marketplace --check all PASS. Orchestrator gate: the three rule-11 cases pass by name. No GAP.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |          |
| 3    | R3        | opus  | 1     | pass          | FINDINGS 0. Seven files, no more (U5's five plus the two wave-1 pack.yaml lines); inventory changed only its two version cells and two composition cells; marketplace.json only stackgen's ref and version; no vwf/site/installer version moved; no surviving 1.6.0 / @1.0.0 in the docs. CONTRACT clean, RULINGS clean.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | —        |
| 3    | gate      | —     | 1     | green         | All eight gate lines green: `plugins:check`, `plugins:marketplace --check`, `plugins:inventory --check`, `plugins:npm-normalize-test`, `pnpm vitest run` (293), `tsc -p installer`, `tsc -p scripts`, `site:check`; both plan checks green. Orchestrator gate: `check.test.ts` shows by name "accepts a Renovate config at the config/ tier root", "flags a root file vwf owns — CLAUDE.md", "flags a root file vwf owns — mempalace.yaml" passing; no target-verifier run — no manifest, skill list or agent change. U5 committed 8b0d1d65 with the two pack.yaml lines.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | 8b0d1d65 |
