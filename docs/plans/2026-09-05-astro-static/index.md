...
Worth ruling in the same breath: U7's first gate-12 fix deleted the `includes`
key from the shipped `.config/dprint.json` (D28's `${configDir}` rewrite does
not work — `includes` is not inherited through `extends` on dprint 0.57.1,
`excludes` is). The pinned plugin list is now the include set, so a materialized
repo also formats `.astro`, `.vue`, `.svelte` and `Dockerfile`, which the
deleted list had drifted away from. The reviewer judged it a gap-fill,
documented in the pack; say so if the narrower set was the intent.
