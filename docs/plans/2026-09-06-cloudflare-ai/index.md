...
this plan. This plan touches `plugins/stackgen/assets/` in exactly one sentence
(`output-tree.md:196`, D21 — a recorded reversal of the standing rule), and
nothing else there.
...
| D20 | Sibling sweep         | Sweep scope: all 24 passages the survey listed (the stale claims, the seven true-but-destinationless passages, the `containers/pack.yaml:45` `EXPOSE` hedge), each fixed in **one clause** in the file's own register, plus a run-time grep for plan C's five slugs against "planned / not offered / not yet / no doctrine / own effort".                                                                                                                                                                                                 | Stale claims only; deferring to a fourth plan                              | U9     |
| D21 | Rulebook carve-out    | A **reversal**, limited to one fact, of the standing "assets are nobody's" rule plans A, B and C all recorded: U9 owns `assets/output-tree.md:196` and `skills/stackgen-stack-template/references/materializer.md:85` and may change nothing else in either file.                                                                                                                                                                                                                                                                         | Leaving them stale; opening `assets/**` generally                          | U9     |
| D22 | Sweep placement       | Wave 3, concurrent with U7 — disjoint Owns, so safe. U8 depends on U7 **and** U9.                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Wave 1                                                                     | U9, U8 |
...
| `plugins/stackgen/assets/**` (taxonomy, kinds, pack-format, contracts)                                                                                      | plan A minted every token; nothing here changes them — `UNRESOLVED:` if a unit thinks it must — **except** the one sentence at `output-tree.md:196` (D21) | nobody; U9 for that sentence only |
| the eighteen older Cloudflare files U9 lists (pipelines, kv, d1, zero-trust, workers-ssr, workers-static-assets, containers `pack.yaml`, `materializer.md`) | the sibling passages every new pack would want to correct                                                                                                 | U9 only, listed passages only     |
...
- **Wave 3 — U7 and U9.** Docs, after every `DOCS FALSIFIED:` line is in; and
  the sibling sweep, after the five packs exist on disk for its run-time grep.
  Disjoint Owns — U7 never touches a pack file, U9 never touches a doc.
- **Wave 4 — U8** (after U7 and U9).** Bump, generate, full gate,
  target-verifier.
...
Plan-specific check, run by the orchestrator after wave 3: U9's seven "none of
the old sentences survives" greps (its Verification block) and its Edit-5 grep
all return nothing.
...
- **Any `plugins/stackgen/assets/` edit** beyond the one sentence D21 carves out
  — plan A's.
