...
| ---------- | ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `vwf`      | `plugins/vwf`      | Skills (slash-invocable workflow skills + auto-applying doctrine skills, incl. the guarded planner `plan`, the ad-hoc planner `change-plan` beside the chain, and the one executor `execute` — both planners writing one plan folder shape, queued in one plan index, which `execute` runs by each unit's `Kind`, taking a folder or `next` — the absorbed `documentation-standards` + `readme` and the vendored `mempalace` + `mempalace-recall` + `karpathy-guidelines`), subagents, the guarded `rtk` Bash hook plus the two mempalace auto-save hooks, and **two** MCP servers — mempalace over **stdio** (see The memory layer: vendored skills, vwf's server) and the absorbed Context7 docs server over stdio. Names **no** technology: no stack templates, no language list                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `stackgen` | `plugins/stackgen` | The **principles-driven stack materializer** — implements the vwf stack-adapter contract with a per-component dispatch rule (a component a shipped pack covers is copied verbatim; an uncovered one is generated behind the `stackgen-skill-reviewer` gate, every concrete third-party name it emits vetted first by `stackgen-reputation` — `pass`, `warn` or `block` from public read APIs, a `block` halting the component until the user names a replacement; the same skill is user-invocable as `/stackgen:stackgen-reputation <ecosystem>:<name> …`), landing in the repo's committed `.claude/` tree plus three consent-gated targets outside it. The only stack plugin, and the home of the repo toolchain, gate and hygiene doctrine since `devtools` dissolved into it — the user-invocable `stackgen:tool-config` owns the mise, dprint, pre-commit, gitleaks and grype configs (a pack asks it for lines through `tool-config:` in its `pack.yaml`), the hygiene pack ships its **config files**, and `/vwf:init` is what lays both down. Its shape, the consent tiers, the pack rules and the inventory pointers are the `stackgen-plugin` skill, which auto-applies under `plugins/stackgen/` |
