...
from project ids, a missing `develop` or `main`, an unfilled repo-name key, a
pack-owned file the repo edited away from the hash the lockfile recorded, an
absent or invalid `MERGE_MODEL`, an absent or empty `MEMBERS` under siblings
linkage. Each row is printed under the repo it was found in, and an **absent**
member is a blind spot rather than a row. A repo behind its baseline still
works, so none of those is ever blocking), **missing** (something declared has
no install — including a **`B`**-kind capability a project declares that none
of its `backing_template` pins provides, which is never blocking; §5),
