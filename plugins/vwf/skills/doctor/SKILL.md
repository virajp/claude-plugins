...
missing from `PATH` or failing its declared probe — the last three
...
or failing its probe while its project's `template` reads `unresolved` (§3),
a declined graph build, and an `iac`
