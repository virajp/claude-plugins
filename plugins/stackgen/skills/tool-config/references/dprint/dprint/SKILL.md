...
and it is a deliberate act run from `setup:mise --upgrade` only — reached by
hand or through `setup:all --upgrade`, in dev only, and never from a plain
`setup:mise` or a plain `setup:all`. Even under `--upgrade` it runs only when
`setup:mise` has a terminal to answer the exec-plugin checksum prompt in. CI and
any non-TTY run skip it by design and print `Skipping formatter plugin updates`,
so a pin moves when a person runs `setup:mise --upgrade` and at no other time.
Pinned plugins are the whole reason two machines format identically.
