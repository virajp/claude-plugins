...
| --------- | ---- | ---- | ---- | -------- | ---------- | ------- | ------- |
| `app-framework/flutter` | Flutter | `app-framework` | project | cross-platform-ui |  | 0.4.1 | The cross-platform app SDK that owns the manifest, the build and the project layout — one codebase across mobile, tablet, desktop and in-car through the native edge. |
...
| `cloud-service/workers-ssr` | Cloudflare Workers SSR | `cloud-provider` | deploy | compute |  | 0.1.2 | A Worker that runs a script in front of its own static assets — on-demand rendering at the edge, the prerendered files served by the platform, and one `wrangler deploy` for both. |
| `cloud-service/workers-static-assets` | Cloudflare Workers Static Assets | `cloud-provider` | deploy | static-hosting |  | 0.1.2 | An assets-only Worker — the build output directory is the whole deployment. No script, no bindings, no server; the edge serves files and `wrangler deploy` uploads them. |
...
| `language/swift` | Swift | `language-bundle` | project |  |  | 0.1.1 | The Swift package baseline — standards and public-API design, the error model, strict concurrency, Swift Testing, build and run, config and observability wiring. |
...
| `package-manager/pnpm` | pnpm | `language-bundle` | repo |  |  | 0.3.1 | Dependency installation, locking and workspace layout for the Node ecosystem — the manifest contract and the monorepo shape. |
...
| `repo-hygiene/repo-hygiene` | Repo hygiene | `repo-hygiene` | repo |  |  | 1.2.3 | The files a repository needs before it has a stack — the ignore set, the editor and attribute defaults, the licence and the security contact, and the dependency-update policy. |
...
| `toolchain-gate/eslint` | ESLint | `language-bundle` | repo |  |  | 0.3.1 | The correctness gate for TypeScript and JavaScript — flat config, zero formatting rules, overrides scoped by glob. |
...
| `toolchain-manager/mise` | mise | `toolchain-manager` | repo |  |  | 1.6.1 | The repo's toolchain manager — the .config/ five-file split selected by MISE_ENV, and the file-based task library everything else invokes, grouped into `setup:*` for bootstrap, `code:*` for gates and git, and `p:<project-id>:*` for a project's own commands. |

## Bundles

| Bundle | Name | Kind | Axis | Components | Unconditional |
| ------ | ---- | ---- | ---- | ---------- | ------------- |
| `astro-csr` | Astro (CSR) | `language-bundle` | project | `language/typescript@0.1.0`, `package-manager/pnpm@0.3.1`, `toolchain-gate/tsconfig@0.2.0`, `toolchain-gate/eslint@0.3.1`, `framework/astro@0.4.0`, `framework/react@generated` |  |
| `astro-hybrid` | Astro (Hybrid) | `language-bundle` | project | `language/typescript@0.1.0`, `package-manager/pnpm@0.3.1`, `toolchain-gate/tsconfig@0.2.0`, `toolchain-gate/eslint@0.3.1`, `framework/astro@0.4.0`, `framework/react@generated`, `framework/effect@0.1.0` |  |
| `astro-ssg` | Astro (SSG) | `language-bundle` | project | `language/typescript@0.1.0`, `package-manager/pnpm@0.3.1`, `toolchain-gate/tsconfig@0.2.0`, `toolchain-gate/eslint@0.3.1`, `framework/astro@0.4.0`, `framework/react@generated` |  |
| `astro-ssr` | Astro (SSR) | `language-bundle` | project | `language/typescript@0.1.0`, `package-manager/pnpm@0.3.1`, `toolchain-gate/tsconfig@0.2.0`, `toolchain-gate/eslint@0.3.1`, `framework/astro@0.4.0`, `framework/react@generated`, `framework/effect@0.1.0` |  |
...
| `cloudflare-containers` | Cloudflare Containers | `cloud-provider` | deploy | `cloud-provider/cloudflare@0.1.0`, `cloud-service/containers@0.1.2` |  |
...
| `cloudflare-workers-ssr` | Cloudflare Workers SSR | `cloud-provider` | deploy | `cloud-provider/cloudflare@0.1.0`, `cloud-service/workers-ssr@0.1.2` |  |
| `cloudflare-workers-static` | Cloudflare Workers Static Assets | `cloud-provider` | deploy | `cloud-provider/cloudflare@0.1.0`, `cloud-service/workers-static-assets@0.1.2` |  |
...
| `dart-flutter` | Dart · Flutter | `app-framework` | project | `app-framework/flutter@0.4.1`, `package-manager/pub@0.1.0`, `toolchain-gate/analysis-options@0.2.0` |  |
...
| `html` | HTML | `language-bundle` | project | `language/typescript@0.1.0`, `package-manager/pnpm@0.3.1`, `toolchain-gate/tsconfig@0.2.0`, `toolchain-gate/eslint@0.3.1`, `framework/html@0.1.0` |  |
| `lovable` | Lovable | `design-tool` | design | `design-tool/lovable@0.1.0` |  |
| `mise` | mise | `toolchain-manager` | repo | `toolchain-manager/mise@1.6.1` | yes |
...
| `pnpm-turbo` | pnpm · Turborepo | `workspace` | repo | `package-manager/pnpm@0.3.1`, `build-orchestrator/turbo@generated` |  |
| `pnpm-workspace` | pnpm · workspace | `workspace` | repo | `package-manager/pnpm@0.3.1` |  |
...
| `repo-hygiene` | repo-hygiene | `repo-hygiene` | repo | `repo-hygiene/repo-hygiene@1.2.3` | yes |
...
| `swift-package` | Swift · package | `language-bundle` | project | `language/swift@0.1.1`, `package-manager/swiftpm@0.1.1`, `toolchain-gate/swift-format@0.1.1`, `toolchain-gate/swiftlint@0.1.1` |  |
...
| `typescript-cloudflare-agents` | TypeScript · Cloudflare Agents · Effect | `language-bundle` | project | `language/typescript@0.1.0`, `package-manager/pnpm@0.3.1`, `toolchain-gate/tsconfig@0.2.0`, `toolchain-gate/eslint@0.3.1`, `framework/effect@0.1.0`, `framework/cloudflare-agents@0.1.0` |  |
| `typescript-effect-cli` | TypeScript · Effect CLI | `language-bundle` | project | `language/typescript@0.1.0`, `package-manager/pnpm@0.3.1`, `toolchain-gate/tsconfig@0.2.0`, `toolchain-gate/eslint@0.3.1`, `framework/effect@0.1.0` |  |
| `typescript-effect-hono` | TypeScript · Hono · Effect | `language-bundle` | project | `language/typescript@0.1.0`, `package-manager/pnpm@0.3.1`, `toolchain-gate/tsconfig@0.2.0`, `toolchain-gate/eslint@0.3.1`, `framework/effect@0.1.0`, `framework/hono@generated` |  |
| `typescript-effect-temporal` | TypeScript · Temporal · Effect | `language-bundle` | project | `language/typescript@0.1.0`, `package-manager/pnpm@0.3.1`, `toolchain-gate/tsconfig@0.2.0`, `toolchain-gate/eslint@0.3.1`, `framework/effect@0.1.0`, `framework/temporal@generated` |  |
| `typescript-effect` | TypeScript · Effect | `language-bundle` | project | `language/typescript@0.1.0`, `package-manager/pnpm@0.3.1`, `toolchain-gate/tsconfig@0.2.0`, `toolchain-gate/eslint@0.3.1`, `framework/effect@0.1.0` |  |
| `typescript-hono-refine` | TypeScript · Hono + Effect · React + Refine | `language-bundle` | project | `language/typescript@0.1.0`, `package-manager/pnpm@0.3.1`, `toolchain-gate/tsconfig@0.2.0`, `toolchain-gate/eslint@0.3.1`, `framework/hono@generated`, `framework/effect@0.1.0`, `framework/react@generated`, `framework/refine@generated` |  |
| `typescript-parseargs-cli` | TypeScript · parseArgs CLI | `language-bundle` | project | `language/typescript@0.1.0`, `package-manager/pnpm@0.3.1`, `toolchain-gate/tsconfig@0.2.0`, `toolchain-gate/eslint@0.3.1` |  |
| `typescript-pulumi` | TypeScript · Pulumi | `language-bundle` | project | `language/typescript@0.1.0`, `package-manager/pnpm@0.3.1`, `toolchain-gate/tsconfig@0.2.0`, `toolchain-gate/eslint@0.3.1`, `framework/pulumi@generated` |  |
