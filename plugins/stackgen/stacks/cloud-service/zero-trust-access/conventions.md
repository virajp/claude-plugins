...
bundle composes with it — including this provider's own three,
`cloudflare-workers-static`, `cloudflare-workers-ssr` and
`cloudflare-containers`, where the proxy and the hosting land in the same
account and are still two separate pins on the axis.
