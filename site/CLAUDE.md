...
| `p:site:version` | `pnpm version <target> --no-git-tag-version` in `site/`, **skipping past a version with a 13 or 17 component** — it computes the target first, stepping the bumped component past the number at the same level, then writes it in that one call and prints what it skipped, so `1.1.12` patched is `1.1.14`; it refuses before writing anything when the level it was given can never reach the forbidden component. Commit `site/package.json` by hand, at the version the task printed |
| `p:site:release` | on `main` only: clean tree, refuses a version carrying a 13 or 17 component, refuses an existing tag, runs `p:site:check`, cuts `site-v<version>`, pushes `main` then the tag, watches `site.yml`                                                                                                                                                                                                                                                                                        |
